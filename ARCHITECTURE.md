...
│
└── install.sh                   # Installs/uninstalls a module
```
...
Declares everything `install.sh` and the auto-discovery system need to know.
...
`requires` lists other module names that must be installed first. `install.sh` checks this before proceeding.
...
`install.sh` regenerates `core/frontend/src/modules.js` after any module change. `main.jsx` imports this file to register routes with React Router. Nav items are also sourced from it.
...
1. **Never edit `modules.js` by hand.** It is generated by `install.sh`.
...
10. **Run `install.sh` for all module changes.** Do not manually pip-install or npm-install module packages.
