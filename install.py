...
def _module_exports(module_dir, name):
    """Return True if the module's frontend index.js exports the given name."""
    index = module_dir / "frontend" / "src" / "index.js"
    if not index.exists():
        return False
    return bool(re.search(rf'\b{re.escape(name)}\b', index.read_text()))
...
def generate_manifest(exclude=None):
    ...
    for module_dir in sorted(MODULES_DIR.iterdir()):
        ...
        if has_navbar_end:
            named_imports += f", NavbarEnd as {name}NavbarEnd"
        imports.append(f"import {{ {named_imports} }} from '@modules/{name}';")
        route_items.append(f"  ...{name}Routes,")
        nav_items.append(f"  ...{name}Nav,")
        if has_providers:
            provider_items.append(f"  ...{name}Providers,")
        if has_navbar_end:
            navbar_end_items.append(f"  {name}NavbarEnd,")
